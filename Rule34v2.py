...
class Rule34(loader.Module):
    ...
    strings = {
        ...
        "invalid_args": "❌ <b>Укажите теги для поиска или настрой default_tags!</b>\n<code>{}r34 теги</code>",
        "loading": "📥 <b>Загружаю медиа...</b>",
        "searching_random": "🎲 <b>Поиск случайного контента...</b>",
        "all_sources_failed": "💥 <b>Все источники недоступны</b>",
        "no_api_key": "<b>❌ Настрой user_id и api_key в </b><code>{}cfg rule34</code>\n<b>Получи их на rule34.xxx в аккаунте (options).</b>",
        "download_error": "<b>❌ Ошибка загрузки медиа</b>",
    }
    ...
    async def _make_request(self, source: Dict, tags: str, limit: int = 50) -> List[Dict]:
        try:
            ...
            if self.config["user_id"] and self.config["api_key"]:
            ...
            async with self._session.get(url, headers=headers) as response:
                ...
                try:
                    ...
                    logger.error(f"{source['name']}: Не JSON ответ")
                    return []
    ...
    def _extract_info(self, post: Dict, source_name: str) -> Optional[Dict]:
        try:
            ...
            rating = self.rating_map.get(post.get("rating", ""), "Unknown")
            info = {
                "id": post_id,
    ...
    def _format_caption(self, info: Dict) -> str:
        tags_list = info.get("tags", [])[:500]
        base_prefix = (
            f"🖼 <b>Источник:</b> {info['source']}\n"
            f"⭐ <b>Score:</b> {info['score']}\n"
            f"📊 <b>Rating:</b> {info['rating']}\n"
            f"🔗 <a href='{info['post_url']}'>Открыть оригинал на сайте</a>\n"
            f"📝 <b>Теги:</b> <blockquote expandable>"
        )
        suffix = "</blockquote>"
        ...
        if self.config["separate_code_tags"]:
            for i, tag in enumerate(tags_list):
                tag_html = f"<code>{utils.escape_html(tag.replace('_', ' '))}</code>"
            ...
            for i, tag in enumerate(tags_list):
                tag_html = utils.escape_html(tag.replace('_', ' '))
                separator = ' ' if current_tags_html else ''
                test_add = separator + tag_html
                if len(current_tags_html + test_add) > available_for_tags:
                    break
    ...
    async def r34(self, message: Message):
        ...
        try:
            include = query
            exclude = self.config["exclude_tags"]
            if self.config["antiai"]:
                exclude += " ai_generated stable_diffusion midjourney novelai ai_art"
            tags = f"{include} -{exclude.replace(' ', ' -')}" if exclude else include

            result = await self._get_random_post(tags)
            ...
            caption = self._format_caption(result)
    ...
    async def r34random(self, message: Message):
        ...
        try:
            query = self.config["default_tags"]
            include = query
            exclude = self.config["exclude_tags"]
            if self.config["antiai"]:
                exclude += " ai_generated stable_diffusion midjourney novelai ai_art"
